...
matrix load_image_augment_paths(char **paths, int n, int min, int max, int size, float angle, float aspect, float hue, float saturation, float exposure, int center)
{
    ...
    X.rows = n;
    X.vals = calloc(X.rows, sizeof(float*));
    memset(X.vals, 0, sizeof(float*) * X.rows);
    X.cols = 0;
//    TIME_BEGIN(crop);
    for(i = 0; i < n; ++i){
//        if(flag) {
//            paths[0]="/home/xuyufan/imagenet_all/ILSVRC2012_img_train/n01739381_1309.JPEG";
//            paths[1]="/home/xuyufan/imagenet_all/ILSVRC2012_img_train/n01739381_1309.JPEG";
//        }
        //TIME_BEGIN(load_image_color);
        image im = load_image_color(paths[i], 0, 0);
        //TIME_END(load_image_color);

        //change
        if(im.data == NULL) {
            matrix res_ = {0,0,NULL};
            free_image(im);
            free_matrix(X);
            return res_;
        }
        ...
        if(center){
            ...
            //TIME_BEGIN(random_augment_image);
            crop = random_augment_image(im, angle, aspect, min, max, size, size);
            //TIME_END(random_augment_image);
        }
        int flip = rand()%2;
        if (flip) flip_image(crop);
        //TIME_BEGIN(random_distort_image);
        random_distort_image(crop, hue, saturation, exposure);
//        if(i==1)
        //TIME_END(random_distort_image);
...
//    d.X = load_image_augment_paths(paths, n, min, max, size, angle, aspect, hue, saturation, exposure, center);
    ...
    //change
    char **temp = paths;
    if(m) paths = get_random_paths(temp, n, m);
    data d = {0};
    d.shallow = 0;
    //TIME_BEGIN(load_image_augment_paths);
    d.X = load_image_augment_paths(paths, n, min, max, size, angle, aspect, hue, saturation, exposure, center);
    //TIME_END(load_image_augment_paths);
    while(0 == d.X.cols){
        free(paths);
        if(m) paths = get_random_paths(temp, n, m);
        d.X = load_image_augment_paths(paths, n, min, max, size, angle, aspect, hue, saturation, exposure, center);
...
//    }
        //change
    //TIME_BEGIN(load_labels_paths);
    d.y = load_labels_paths(paths, n, labels, k, hierarchy);
    //TIME_END(load_labels_paths);
    if(m) free(paths);
    return d;
}
